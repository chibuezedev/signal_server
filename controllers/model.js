...
router.post('/process_sentence', async (req, res) => {
  ...
  if (!Array.isArray(sentence)) {
    return res.status(400).json({ error: 'Sentence must be an array of strings' });
  }
  ...
  try {
    ...
    const result = await model.generateContent(`Process and improve the following sentence translated from sign language: ${sentenceStr}`);
    const response = await result.response;
